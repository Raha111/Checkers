...
board_evaluation = ctrl.Antecedent(np.arange(-20, 21, 1), 'board_evaluation')
...
board_evaluation['weak'] = fuzz.trimf(board_evaluation.universe, [-20, -10, 0])
board_evaluation['strong'] = fuzz.trimf(board_evaluation.universe, [0, 10, 20])

move_strength['weak'] = fuzz.trimf(move_strength.universe, [0, 25, 50])
move_strength['strong'] = fuzz.trimf(move_strength.universe, [50, 75, 100])
...
def calculate_fuzzy_move(board, row, col):
    ...
    if piece:  # Ensure piece is not None and not zero (empty square)
        ...
        board_evaluation_value = board.evaluate()  # Ensure evaluate method is correct in Board class
        ...
        '''
        # Check if inputs are within the expected range for the fuzzy system
        try:
            move_simulation.compute()
...
def determine_best_fuzzy_move(board):
    ...
    for row in range(ROWS):
        for col in range(COLS):
            piece = board.get_piece(row,col)
            if piece != 0 and piece.color == WHITE:  # Assuming AI controls WHITE pieces
